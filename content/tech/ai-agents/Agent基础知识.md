---
title: "Agent基础知识"
date: 2026-04-20T16:30:41+08:00
draft: false
description: ""
categories: ["技术"]
tags: []
...
> **原文来源**：https://mp.weixin.qq.com/s/lnXbPXCPQBkgGqlvRJr9KA

现在无论是什么岗位，都要求了解一些AI，Agent相关的内容。

从25年开始，[知识星球](https://mp.weixin.qq.com/s?__biz=MzUxNjY5NTYxNA==&mid=2247523232&idx=3&sn=98fec2d2c68f22d0cb2e11ec6692bd96&scene=21#wechat_redirect)里就有录友开始反馈，很多岗位要求有agent经验，而且在面试的过程中会主动问你是否了解agent。
...
![图1：LLM vs Agent 对比图](images/Agent基础知识/1.jpg)
...
![图2：Workflow vs Agent 流程对比图](images/Agent基础知识/2.jpg)

## 图2：Workflow vs Agent 流程对比图
...
同样拿退款举例：用户说"处理这个退款"，Agent 自己思考——"我需要先了解申请内容"，于是调用 read\_ticket()；"发现是高价值订单，需要查特殊政策"，于是调用 search\_policy\_doc()；"政策允许，但需要主管审批"，于是调用 create\_approval\_request()。
...
![图3：ReAct 工作模式循环图](images/Agent基础知识/3.jpg)
...
![Plan-and-Execute 两阶段工作模式](images/Agent基础知识/4.jpg)

## Plan-and-Execute 两阶段工作模式
...
![Reflection 自我反思工作模式](images/Agent基础知识/5.jpg)
...
![图4：Function Call 四步工作流程图](images/Agent基础知识/7.jpg)

## 图4：Function Call 四步工作流程图
...
| --- | --- | --- |
| 工具定义 | tools + function | tools + input\_schema |
| 调用输出 | tool\_calls 数组 | tool\_use content block |
| 结果传回 | role: "tool" | role: "user" + tool\_result content block |
...
![图5：MCP 架构对比图](images/Agent基础知识/8.jpg)
...
![图6：Function Call、MCP、Skills 三层分工图](images/Agent基础知识/9.jpg)
...
1. **Skills 匹配**：检测到"审查"关键词 → 加载 Code\_Review Skill → "好，我知道审查要关注安全/性能/质量三个维度"
...
![图7：A2A + MCP 完整协议生态图](images/Agent基础知识/10.jpg)
...
![图8：Agent 记忆系统分层图](images/Agent基础知识/11.jpg)
...
![Agent 安全防御体系](images/Agent基础知识/12.jpg)
