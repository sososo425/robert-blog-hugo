...
#### ADR-AMS-001：为什么选择单入口而非 Agent 直连存储？

**直连存储的诱惑**：Agent Framework 直接访问 Redis（Working Memory）和 Neo4j/Milvus（Long-term Memory），减少一次网络跳转，延迟看起来更低。

**选择单入口的三个根本原因**：

| **原因** | **说明** | **若无 AMS 的后果** |
|---|---|---|
| **① 多租户隔离的强制执行点** | `tenant_id` 过滤、RLS 策略、Key 前缀注入必须在一个可信的服务端统一执行，不能依赖客户端自律 | 客户端 Bug 或恶意调用可读取其他租户的记忆数据 |
| **② 降级策略的统一调度** | Milvus 宕机时切换检索通道、LLM 不可用时 Agentic Mode 降级为 Lightweight——这些需要全局状态感知，单个 Agent 实例看不到整体 | 每个 Agent 各自处理降级逻辑，行为不一致，部分 Agent 直接报错 |
| **③ 记忆生命周期的全局视角** | Decay 引擎、Session Promotion、Novelty 去重都需要跨 Agent、跨会话的全局数据，而不是某个 Agent 实例的局部视角 | 无法执行跨 Agent 的记忆去重；Decay 引擎必须直连每个存储引擎，运维复杂度爆炸 |

> **延迟代价**：单入口引入约 5~15ms 的额外网络跳转（同 AZ 内 gRPC 调用）。在 Working Memory 的 p99 < 50ms 目标下，这是可接受的。对于 Long-term Memory 的 p95 < 200ms 目标，AMS 本身不做计算，额外开销可忽略。

### 1.2 职责边界（RACI 矩阵）

| **职责** | **AMS** | **Pipeline Workers** | **Storage Engines** | **Agent-TES** | **Skill-DOM** |
|---|:---:|:---:|:---:|:---:|:---:|
...
### 2.3 上下文组装策略（Context Assembly Strategy）

Context Builder 是 AMS 对 Agent 最直接的价值输出点——它决定了"哪些记忆以什么形式出现在 LLM 的输入里"。不合理的组装会导致两类问题：① Context 窗口溢出，关键信息被截断；② 注入了大量低相关记忆，稀释有效信息密度，反而干扰 LLM 的判断。

#### 2.3.1 Token 预算分配模型

以 128K context window 的 LLM（如 GPT-4o）为例，AMS 采用**分层预算分配**：

```
┌──────────────────────────────────────────────────────────────┐
│               Context Window（128K tokens 总预算）            │
│                                                              │
│  ┌────────────────────┐  ~2K   System Prompt（固定，最高优先级）│
│  ├────────────────────┤                                      │
│  │  LTM 检索结果       │  ~20K  动态，按 final_score 截断      │
│  │  (Long-term Mem)   │        超出则丢弃低分结果              │
│  ├────────────────────┤                                      │
│  │  WM 对话历史        │  ~50K  保留最近 N 轮，从最老一轮开始丢 │
│  │  (Working Memory)  │        弃，直至满足预算               │
│  ├────────────────────┤                                      │
│  │  当前 User Message  │  ~4K   不可裁剪                      │
│  ├────────────────────┤                                      │
│  │  工具调用结果/预留   │  ~52K  剩余空间，供 Agent 输出使用    │
│  └────────────────────┘                                      │
└──────────────────────────────────────────────────────────────┘
```

> **注意**：上述数字是系统默认值，可通过 Agent 级配置 `context_budget` 覆盖。不同 LLM 的 context window 大小不同，AMS 在创建 Session 时从 Agent 配置中读取 `max_context_tokens`，动态计算各分区预算。

#### 2.3.2 各来源内容的优先级与裁剪规则

| **优先级** | **内容来源** | **裁剪策略** | **裁剪单位** |
|:---:|---|---|---|
| 1（最高） | System Prompt | 不裁剪，超出则报错 | — |
| 2 | 当前 User Message | 不裁剪，超出则返回 E1003 | — |
| 3 | LTM 检索结果 | 按 `final_score` 降序，丢弃超出预算部分 | 单条 memory result |
| 4 | WM 对话历史 | 从最早一轮开始丢弃，保留最近 N 轮 | 单轮对话（user+assistant 一对） |
| 5（最低） | 工具调用结果 | 超出时截断 result 字段，保留 tool_name + status | 字段级截断 |

**LTM 结果的 token 预算分配细节**：

```python
def allocate_ltm_budget(results: list[MemoryResult], budget_tokens: int) -> list[MemoryResult]:
    """
    从高分到低分依次纳入，直到 token 预算耗尽。
    每条 result 包含：summary（必选）+ content_blocks（可选）+ graph_context（可选）。
    若单条 summary 已超出剩余预算，则跳过该条。
    """
    selected = []
    ...
    for result in sorted(results, key=lambda r: r.final_score, reverse=True):
        ...
        if summary_tokens > remaining:
            continue  # 连 summary 都放不下，跳过

        allocated = result.copy_with_only_summary()
        remaining -= summary_tokens

        # 按优先级追加可选内容：content_blocks > graph_context
        for block in result.content_blocks:
            block_tokens = count_tokens(block.content)
            if block_tokens <= remaining:
                allocated.add_block(block)
                remaining -= block_tokens

        selected.append(allocated)
...
Context Builder 支持三种输出格式（对应 API 的 `output_config.format`）：

| **格式** | **适用场景** | **示例** |
|---|---|---|
| `structured` | Agent 需要程序化处理记忆数据（默认） | JSON 对象，含 tree_context / graph_context 字段 |
| `flat_text` | 直接嵌入 LLM prompt 的纯文本 | `[Memory: Q3销售分析] A产品线同比+22%...` |
| `markdown` | 富文本展示或中间件消费 | Markdown 格式，含标题、表格 |

**`flat_text` 格式模板**（供参考）：

```
--- Relevant Memories ---
[1] {title} (relevance: {relevance_score:.2f}, from: {source.name})
{summary}
...
| AMS → Skill-DOM | gRPC | 同步 | Protobuf | 技能检索查询 |
...
│              ▼            ▼            ▼                        │
│     ┌──────────────┐ ┌──────────┐ ┌────────────────┐           │
│     │ DeclarativeRecord│ │Procedural│ │MetacogRecord   │           │
│     │ (陈述性记忆)     │ │Record    │ │(元认知记忆)     │           │
│     │               │ │(程序记忆) │ │                │           │
│     └──────────────┘ └──────────┘ └────────────────┘           │
│                                                                 │
...
CREATE TABLE memory_records (
    ...
    -- 记忆分类
    memory_type     VARCHAR(30) NOT NULL,  -- 'declarative' / 'procedural' / 'metacognitive'
    memory_subtype  VARCHAR(30),           -- declarative: 'semantic' / 'episodic'; procedural: 'workflow' / 'skill' / 'pattern'
    ...
    -- structures_status 跟踪每种结构的构建状态，解决并发更新问题（见下方设计说明）
    structures_status JSONB NOT NULL DEFAULT '{}',
    -- 示例值: {"tree": "completed", "graph": "processing"}
    ...
    -- 约束
    CONSTRAINT chk_memory_type CHECK (memory_type IN ('declarative', 'procedural', 'metacognitive')),
...
> **`structures_status` 并发更新设计说明**
>
> `structures` 字段标识一条记忆"应该"拥有哪些结构（`['tree', 'graph']`），而 `structures_status` 字段跟踪每种结构当前的**构建进度**。
>
> 当一条 `work_document` 类型的记忆被摄入时，Pipeline 会同时启动 Tree Construction 和 Graph Construction 两个并发任务。两个 Worker 可能同时尝试更新 `memory_records` 表的同一行。`structures` 字段是 Array 类型，两个 Worker 各自执行 `array_append` 会导致竞态条件。
>
> **解决方案**：通过 JSONB 的 key-level 原子更新，每个 Worker 只更新自己负责的 key：
>
> ```sql
> -- Tree Worker 完成时只更新 tree 的状态，不触碰 graph
> UPDATE memory_records
> SET structures_status = structures_status || '{"tree": "completed"}'::jsonb,
>     updated_at = now()
> WHERE memory_id = $1;
>
> -- Graph Worker 完成时只更新 graph 的状态
> UPDATE memory_records
...
> ```
>
> JSONB 的 `||` 合并操作在 PostgreSQL 层面是行级锁保护的，两个并发更新不会互相覆盖，只会各自写入自己的 key。记忆的"完整可用状态"定义为 `structures_status` 中所有应有的 key 均为 `"completed"`。
```
...
('user_interaction',  ARRAY['procedural','declarative'], ARRAY['graph'],        'Preserves relational patterns and conversational flow as navigable graph paths'),
('agent_trace',       ARRAY['procedural','declarative'], ARRAY['graph'],        'Models execution trajectories and tool chains as temporal graph paths'),
('work_document',     ARRAY['procedural','declarative'], ARRAY['tree','graph'], 'Requires hierarchical abstraction AND entity-relationship graphs'),
('source_code',       ARRAY['procedural','declarative'], ARRAY['tree','graph'], 'Call Graph and Dependency Tree for code'),
...
// Block（内容块节点）—— Tree↔Graph 桥接的关键节点
// Block 同时存在于 Tree 结构（作为叶子节点）和 Graph 结构（作为 HAS_CONTEXT 边的起点），
// 是两种结构双向互达的"桥梁"。通过 Block 可以从 Graph 的 PhraseNode 回溯到原始文本，
// 也可以从 Tree 的段落跳入 Graph 的实体关系网络。
// Labels: :Block
(:Block {
    block_id:       String,     // UUID，与 PG tree_blocks 表主键对应
    tenant_id:      String,     // 租户隔离
    agent_id:       String,     // Agent 归属
    memory_id:      String,     // 所属的 memory_record UUID
    section_id:     String,     // 所属 Section L1 的 section_id

    block_type:     String,     // 'text' | 'code' | 'table' | 'img'
    position:       Integer,    // 在所属 Section 内的顺序位置（0-based）

    // 内容摘要（完整 raw_text 存储在 PG/ES，此处仅存 preview 用于图遍历时的快速预览）
    content_preview: String,    // raw_text 的前 200 字符
    summary:        String,     // LLM 生成的摘要
    tags:           String[],   // 标签数组

    // 生命周期（与 memory_record 保持同步）
    decay_weight:   Float,
...
// Block 约束与索引（补充）
CREATE INDEX block_memory IF NOT EXISTS
FOR (n:Block) ON (n.memory_id);

CREATE INDEX block_section IF NOT EXISTS
FOR (n:Block) ON (n.section_id);

CREATE INDEX block_tenant IF NOT EXISTS
FOR (n:Block) ON (n.tenant_id);

// PhraseNode（短语节点）—— Graph 的基本单元
// Labels: :PhraseNode[:Declarative|:Procedural]
(:PhraseNode {
    node_id:        String,     // UUID
    tenant_id:      String,     // 租户隔离
    agent_id:       String,     // Agent 归属
    memory_type:    String,     // 'declarative' | 'procedural'
...
AMS 将向量索引拆分为四个独立的 Collection，而非用一个大 Collection 存储所有 embedding。这一设计对应**四种不同的检索粒度与查询意图**：

| **Collection** | **粒度** | **对应的查询意图** | **在检索流程中的触发位置** |
|---|---|---|---|
| `block_embeddings` | 段落级（精细） | "找到与 query 最相关的原文段落" — 精准语义匹配，召回最直接的证据 | Lightweight/Agentic 的 Vector Channel 主力召回 |
| `section_embeddings` | 章节级（中等） | "找到与 query 相关的主题章节" — 快速定位大方向，减少 block 全量扫描开销 | 可选的粗召回阶段，或用于 Tree Search 的入口定位 |
| `node_embeddings` | 实体级（Graph 入口） | "找到与 query 相关的实体/概念节点" — 图检索的语义种子，找到后再通过边遍历邻居 | Graph Channel 的入口：先向量找种子节点，再 PPR 扩展 |
| `community_embeddings` | 社区级（高层摘要） | "query 涉及哪个主题社区" — 回答"关于 X 的总体情况"类全局问题 | Agentic Mode 的 Query Expansion 后的高层召回 |

> **设计哲学**：四个 Collection 构成从"精细证据"到"主题概览"的召回谱系，在检索时可以按 query 意图的宏观程度选择性触发。简单事实查询优先用 `block_embeddings`；全局主题查询优先用 `community_embeddings`；需要关系推理时，`node_embeddings` 是图遍历的语义入口。
...
block_embeddings_schema = CollectionSchema(
    fields=[
        ...
        FieldSchema(name="memory_type",     dtype=DataType.VARCHAR, max_length=20),   # 'declarative' | 'procedural'
...
> |---|---|
> | 查询是单跳事实性问题（"张三的职位是什么"、"Q3销售额是多少"） | 查询需要多跳推理（"比较A和B在过去三个季度的表现趋势变化"） |
> | 延迟敏感，SLA < 300ms | 可以接受 1~3 秒延迟，质量优先 |
> | 检索结果将直接嵌入 LLM context，无需进一步推理 | 查询语义模糊，需要扩展或分解（"最近有什么重要进展"） |
> | 高并发场景，需要稳定的吞吐量 | 复杂分析任务，需要跨多条记忆整合信息 |
>
> **经验规则**：当你不确定时，从 `lightweight` 开始。若返回结果的 `retrieval_metadata.avg_relevance_score` 持续低于 0.5，再考虑切换到 `agentic`。
...
Request Body:
{
  ...
  // ─── 过滤条件 ───
  "filters": {
    "memory_types": ["declarative", "procedural"],          // 限定记忆类型
    "memory_subtypes": ["semantic", "episodic"],              // 限定子类型（当 memory_type=declarative 时）
...
Request Body:
{
  ...
  "memory_type": "declarative",                              // required: "declarative" | "procedural" | "metacognitive"
  "memory_subtype": "semantic",                               // optional: declarative: "semantic" | "episodic"; procedural: "workflow" | "skill" | "pattern"
...
Response 200 OK:
{
  ...
  "stats": {
    ...
    "long_term_memory": {
      "total_memories": 1250,
      "by_type": {
        "declarative": { "total": 980, "active": 920, "decayed": 45, "archived": 15 },
...
Request Body:
{
  ...
  "processing_config": {
    ...
    "tree_config": {
      ...
      "chunk_strategy": "semantic",                           // 分块策略："semantic" | "fixed_size" | "heading_based"（与记忆类型 semantic/episodic 无关）
      "chunk_size": 512,                                      // 仅 fixed_size 时生效
      "overlap": 50
    },
    "graph_config": {
      // layer_mode 选择规则（与 source_type 强关联，违反映射规则返回 E1001）：
      //   "double_layer"    → 适用于静态文档类：work_document / source_code
      //                        构建 Phrase Node → Community Node 两层图
      //                        不含 Temporal KG（静态文档无时序语义）
      //   "full_three_layer"→ 适用于动态数据类：user_interaction / agent_trace
      //                        构建 Phrase Node → Community Node → Temporal KG 三层图
      //                        Temporal KG 捕获执行轨迹的时序关系，是回溯 Agent 行为的关键
      //   skill_markdown 不使用 graph_config（该 source_type 只走 Tree，Graph 字段忽略）
      "layer_mode": "double_layer",                           // "double_layer" | "full_three_layer"
...
Request Body:
{
  ...
  "decay_config": {
    "lambda": 0.001,                                          // 衰减系数（0.001 → ~2年降至0.5）
    "min_weight": 0.05,                                       // 低于此值标记为 decayed
    "archive_threshold": 0.01,                                // 低于此值标记为 archived
    "access_boost_enabled": true,                             // 访问频率是否抵消衰减
    "boost_days": 10                                          // 每次访问等效延缓衰减的天数
  }
}
...
│  │    b. 任务完成度: 是否有成功的工具调用链                            │
│  │    c. 用户满意度: 是否有正向反馈信号                                │
│  │    d. 新颖性 (Novelty Score): 与已有记忆的语义距离                  │
│  │       → 对会话的 Agent 输出部分生成 embedding                       │
│  │       → 在 Milvus block_embeddings 中检索 Top-3 最相似已有记忆      │
│  │       → novelty_score = 1 - max_cosine_similarity(top_3)          │
│  │       → 特殊处理：若 max_cosine_similarity > 0.92（近似重复）       │
│  │           则跳过新建，仅对已有记忆执行 access_count++ 和 importance  │
│  │           轻微上调（+0.02），避免语义重复的记忆碎片化累积             │
│  │  → promotion_score = weighted_sum(a×0.3, b×0.3, c×0.2, d×0.2)    │
│  │  → if promotion_score < threshold: DISCARD                        │
│  │                                                                    │
│  ③ 记忆提取与分类                                                     │
│  │  → LLM 从会话内容中提取值得记忆的信息                              │
│  │  → 分类为 declarative (semantic/episodic) 或 procedural (workflow/pattern) │
...
│  │                                                                    │
│  ② 计算新的衰减权重                                                   │
│  │  → 基础衰减（Ebbinghaus 遗忘曲线变体）:                             │
│  │    w_base(t) = e^(-λ × effective_age_days)                        │
│  │    其中 effective_age_days = max(0, days_since_creation            │
│  │                                   - access_count × boost_days)    │
│  │    默认 boost_days = 10                                            │
│  │    → 含义：每次被检索访问，等效于记忆"年轻了 10 天"                  │
│  │    → 符合认知科学的间隔重复（Spaced Repetition）理论：              │
│  │      被频繁使用的记忆衰减更慢，反映其持续的价值                      │
│  │                                                                    │
│  │  λ 参数业务直觉（λ = 0.001）:                                      │
│  │    → 从未被访问的记忆，约 693 天（~2 年）衰减权重降至 0.5           │
│  │    → 约 4605 天（~13 年）降至接近 0（< 0.01 触发 archived）        │
│  │    → 每访问 1 次 ≈ 延缓约 10 天的衰减                              │
│  │                                                                    │
│  │  → 重要性保护:                                                      │
│  │    w_importance = importance ^ 0.5  (高重要性记忆衰减更慢)          │
│  │    → importance=1.0 → w_importance=1.0（无额外衰减）               │
│  │    → importance=0.25 → w_importance=0.5（衰减加速一倍）            │
│  │                                                                    │
│  │  → 最终权重:                                                        │
│  │    decay_weight = min(1.0, w_base × w_importance)                 │
│  │    注：access 频率已通过 effective_age 偏移融入 w_base，            │
│  │    不再单独乘以 (1+w_access) 系数，避免高访问量下溢出至 1.0 上限    │
│  │    导致区分度丧失。                                                  │
│  │                                                                    │
│  ③ 状态转换                                                           │
...
**分区键设计原则**：分区键决定了同一逻辑实体的消息是否能被同一 Consumer 实例顺序消费。对于需要"有状态拼接"的消息（如同一会话的多条 Trace），必须用业务 ID 作为分区键，确保顺序性；对于无状态的任务消息，可以用 `memory_id` 保证同一记忆的多阶段 Pipeline 串行，避免并发写冲突。

| **Topic** | **Producer** | **Consumer** | **分区数** | **副本数** | **保留期** | **分区键** | **说明** |
|---|---|---|---|---|---|---|---|
| `ams.trace.ingested` | Agent-TES | AMS (Router) | 12 | 3 | 7d | `session_id` | 同一会话 Trace 需按时间序拼接，必须顺序消费 |
| `ams.session.archive` | AMS (Session Mgr) | Pipeline Workers | 6 | 3 | 3d | `agent_id` | 同一 Agent 的会话 Promotion 串行执行，避免并发写冲突 |
| `ams.pipeline.ingestion` | AMS (Router) | Pipeline Workers | 12 | 3 | 3d | `memory_id` | 同一记忆的多阶段 Pipeline 必须顺序执行 |
| `ams.pipeline.transform` | Pipeline Workers | Pipeline Workers | 12 | 3 | 3d | `memory_id` | 同上 |
| `ams.pipeline.structure` | Pipeline Workers | Pipeline Workers | 12 | 3 | 3d | `memory_id` | 同上 |
| `ams.pipeline.indexing` | Pipeline Workers | Pipeline Workers | 12 | 3 | 3d | `memory_id` | 同上 |
| `ams.pipeline.dlq` | Pipeline Workers | Ops Team | 3 | 3 | 30d | `memory_id` | 死信队列，保留完整链路用于排查 |
| `ams.memory.events` | AMS | Skill-DOM / Analytics | 6 | 3 | 7d | `agent_id` | 同一 Agent 的记忆事件保序，方便下游按 Agent 聚合 |
| `ams.feedback` | AMS | Analytics Workers | 3 | 3 | 30d | `tenant_id` | 反馈数据按租户聚合分析 |
...
long_term_memory:
  ...
  decay:
    ...
    lambda: 0.001                                   # λ=0.001 → 未访问记忆约 693 天降至 0.5
    min_weight: 0.05
    archive_threshold: 0.01
    access_boost_enabled: true
    boost_days: 10                                  # 每次访问等效延缓衰减 10 天
    batch_size: 1000
...
| **Neo4j 不可用** | Tree + Graph 通道 + 上下文组装均失效 | ① 自动禁用 tree/graph channel，仅 BM25 + Vector 双路召回；② 上下文组装阶段强制将 `include_tree_path` 和 `include_graph_context` 降级为 `false`，返回的结果只包含 `summary` 和 `source` 信息，不返回结构化上下文 | 多跳推理和层级导航不可用；检索结果缺少 Tree 路径和 Graph 关联实体 |
...
> **Neo4j 降级补充说明**：Neo4j 不可用时的两阶段降级——召回阶段和上下文组装阶段都依赖 Neo4j，必须同时处理。若只禁用了召回通道而未处理组装阶段，Step ⑥ 的 `neo4j.fetch_tree_path` 和 `neo4j.fetch_graph_context` 调用仍会超时，导致整个检索请求失败，与"优雅降级"的目标背道而驰。
...
ams_wm_ltm_token_utilization{tenant_id, agent_id}                      histogram    # LTM 检索结果占 context window 的 token 比例
                                                                                    # 若长期接近 0 → auto_retrieve 阈值过高，无记忆被注入
                                                                                    # 若长期接近 1 → LTM 结果撑满 context，对话历史被严重压缩
ams_wm_ltm_memories_injected{tenant_id, agent_id}                      histogram    # 每次 AppendMessages 实际注入的 LTM 记忆条数
...
ams_promotion_promoted_total{tenant_id, memory_type}                   counter  # memory_type: declarative/procedural/metacognitive
...
| **服务间认证** | mTLS | AMS 与 Pipeline Workers、Skill-DOM 之间的 gRPC 通信 |
| **存储认证** | 密码/证书（K8s Secret） | 所有存储引擎连接使用独立的服务账号 |

> ⚠️ 命名一致性：本文档中 Skill 子系统统一使用 **Skill-DOM**（与概要设计一致）。
...
**Milvus 向量数与 ES 文档数按数据源分类估算**：

并非所有记忆都产生 block_embeddings——`user_interaction` 和 `agent_trace` 类型只走 Graph Construction，不产生 Block，因此不产生 block/section 层的向量和 ES 文档。只有文档类数据源才需要 Tree 结构的向量索引。

| **数据源** | **占比** | **年记忆条数** | **avg blocks/memory** | **年 Block 向量数** |
|---|---|---|---|---|
| `user_interaction` | 50% | 18.25M | 0（Graph only） | 0 |
| `agent_trace` | 20% | 7.3M | 0（Graph only） | 0 |
| `work_document` | 25% | 9.1M | 15 | ~137M |
| `skill_markdown` | 5% | 1.83M | 5 | ~9M |
| **合计** | 100% | **36.5M** | — | **~146M/year** |

此外，所有记忆类型都会产生 node_embeddings（Graph 的 PhraseNode）和 community_embeddings：

| **向量 Collection** | **估算年向量数** | **备注** |
|---|---|---|
| `block_embeddings` | ~146M | 仅文档类数据源 |
| `section_embeddings` | ~15M | 约为 block 数的 1/10 |
| `node_embeddings` | ~547M | 所有记忆类型均产生 PhraseNode |
| `community_embeddings` | ~35M | 约为 node 数的 1/15（社区聚合率） |
| **ES 文档数（ams_blocks）** | ~146M | 与 block_embeddings 一致 |

> **修正说明**：原始估算 `36.5M × 10 = 365M` 没有区分数据源类型，导致 block 向量数高估约 2.5 倍（365M vs 146M）。这对 Milvus 的存储规划有显著影响：以 `text-embedding-3-small`（1536 维，float32）计算，146M 向量约需 **876 GB** 原始向量存储（不含索引开销），而非 2.2 TB。在 Milvus 的 HNSW 索引下，实际磁盘需求约为原始向量的 1.5~2 倍，即 **~1.3~1.8 TB**，与规划的 3 个节点各 1 TB SSD 相符。
