...
### 1.5 原型范围与实现阶段（与仓库实现对齐）

本节吸收评审文档《改进建议-opus-review》《改进建议-kimi-review》中与「可落地原型」相关的结论，作为**当前代码迭代**的设计门禁；完整生产实现仍以下文各章为准。

#### 1.5.1 六类问题的阶段划分（工程优先级）


| 阶段  | 覆盖类型                                     | 说明                                   |
| --- | ---------------------------------------- | ------------------------------------ |
| P0  | 类型 3（时间线重建）、类型 4（因果追溯）                   | 证明 Session–Trace 桥接价值；本仓库 AMS 原型优先交付 |
| P1  | 类型 1（实体查询）、类型 2（路径查询）                    | 实体邻居与限定跳数路径子图                        |
| P2  | 类型 5（跨会话指代）、类型 6（语义联想 / SemanticCluster） | 依赖聚类与向量检索质量                          |


#### 1.5.2 跨 Neo4j / Milvus 一致性（原型声明）

- **写入顺序**：先 **Neo4j 事务提交成功**，再写 **Milvus** 辅助索引；Milvus 失败时记录日志并进入重试队列占位（原型可为 JSON 记录）。
- **产品语义**：Milvus 用于实体归一候选召回与语义检索，允许**短暂最终一致**；权威状态以 Neo4j 为准。

#### 1.5.3 桥接边与 PRECEDES（原型可操作规则）

- 桥接边构建为 Pipeline **独立子步骤**（Stage 4.x），参数可配置：`time_window_sec`、`require_entity_overlap`（布尔）。
- `PRECEDES`：为 `EventNode` 建立索引 `(tenant_id, session_id, event_time)`，或在写入时维护 **上一事件 event_id** 指针以降低大范围排序扫描（二选一，性能优化项）。

#### 1.5.4 其他建模取舍（原型期）

- **UserProfile**：不引入独立节点；用户画像字段挂在 `EntityNode`（`entity_type=PERSON`）或整体延后。
- **ACTION 实体**：弱化；优先用 `EventNode` + `PARTICIPATES_IN` 表达动作语义，减少与 `tool_use` 触发文本重叠。
- **EventNode.embedding**：原型支持**可选持久化**属性，供 SemanticCluster；未填时在聚类阶段对 `summary` 临时向量化（依赖 Embedding 服务）。
- **遗忘与画像**：原型期画像可采用**滑动窗口式**统计表述占位；不与完整遗忘模块耦合。
- **Sufficiency Loop**（EverMemOS 式）：P3，仅预留检索 API 参数（如 `expand_hops`、`min_similarity`）作为扩展点。
- **多租户**：原型为 **tenant_id 逻辑隔离**；物理隔离（多库/分片）列为演进路径。
- **混合抽取 Layer1/2/3**：与第2章架构图、第5章一致；Layer1 负责 Schema 内封闭标签（Encoder / GLiNER-RelEx / BERT 类）；Layer2 为规则与轻量工具；Layer3 负责事件、开放关系与纠错。Layer2 输出时间解析时保留**原始文本片段**供 Layer3 语境校准；可标记 `entity_mismatch` 触发二次策略或人工审核。仓库实现说明与 `my_tools/bert-test` 对齐指引见 `docs/ams-hybrid-extraction.md`。
- **EntityNode**：可增加可选 `schema_version` 字段便于类型体系统一升级。

#### 1.5.5 与既有仓库模块的关系（内部原型阶段）

当前无线上用户，**管理台与 API 以 AMS 主线收敛即可**；下列为工程上的兼容与迁移说明，而非「必须长期并存」的产品承诺。

- **G3Demo***：历史外部 demo 写入的标签集合，与 AMS 正式 `EntityNode` / `EventNode` / `RawBlockRef` **数据不混用**；管理台「记忆构建验证」页可逐步下线或改为跳转 AMS 实验台。
- `**/semantic`（PhraseNode / Milvus blocks）**：版本 B/C GraphRAG 实验路线；可与 AMS **合并能力**（统一入口、统一 UI）或保留作对照实验，由团队自行决定，不要求为虚构用户维持双栈。
...
- 从原始数据输入到知识图谱写入和语义固化的完整 pipeline（**Stage 2 为混合抽取**，详见第5章；Encoder 选型与本地验证可参考 `docs/ams-hybrid-extraction.md`）
...
flowchart TD
    ...
    %% ===== Stage 2 混合抽取（第5章）=====
    S1 --> L1
    subgraph S2["Stage 2 · Hybrid Extraction<br/>Encoder + Rules + LLM"]
        direction TB
        L1["Layer 1 · Encoder<br/>GLiNER-RelEx / BERT-class<br/>封闭域实体 + 预定义关系"]
        L2["Layer 2 · Rules<br/>时间解析 · 别名/噪声过滤<br/>与 Trace 字段对齐"]
        L3["Layer 3 · LLM Refine<br/>事件 EventNode · 开放关系<br/>Foresight valid_at / invalid_at"]
        L1 --> L2 --> L3
    end

    %% ===== Stage 3 =====
    L3 --> S3["Stage 3 · 实体去重与链接<br/>规则归一化 → 向量候选召回（Milvus）<br/>→ LLM Candidate-ID 判断<br/>离线 Entity Normalization（SYNONYM_OF 边，批处理）<br/>仅处理 EntityNode，EventNode 直通 Stage 4"]
    ...
    style L1 fill:#aed581,stroke:#558b2f,color:#111
    style L2 fill:#fff59d,stroke:#f9a825,color:#111
    style L3 fill:#F5A623,stroke:#D4891A,color:#fff
...
-- source_block_ids  LIST<STRING>                    -- 关联的 RawBlock ID 列表
